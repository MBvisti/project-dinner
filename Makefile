...
mail_provider_usr := MAIL_PROVIDER_USER=9b654c6340c05a
mail_provider_key := MAIL_PROVIDER_API_KEY=e398d948d22555
...
dev-build: vet format
	...
	@docker run --rm -e WHAT_ENVIRONMENT_IS_THIS=development -e DEVELOPMENT_MODE=true -e MAIL_PROVIDER_USER=9b654c6340c05a -e MAIL_PROVIDER_API_KEY=e398d948d22555 -e HOST=smtp.mailtrap.io -e MAIL_PORT=25 -e IS_STAGING=true -e DATABASE_URL="postgres://postgres:postgres@host.docker.internal/project_dinner_dev?sslmode=disable" -p 5000:5000 ${REGISTRY}/${OUT}:${VERSION}
...
run-dev:
	./start.sh ${what_env} ${mail_provider_usr} ${mail_provider_key} ${host} ${mail_port} ${is_staging} ${db_url}
